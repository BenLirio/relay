...
const BlogPost = ({ data }) => {
  ...
  const renderNodes = (node) => {
    ...
    switch (node.tagName) {
      ...
      case 'H1': {
        return <Typography variant="h1" key={key}>{children}</Typography>
      }
      case 'H2': {
        return <Typography variant="h4" key={key}>{children}</Typography>
      }
      case 'H3': {
        return <Typography variant="h4" key={key}>{children}</Typography>
      }
      case 'H4': {
        return <Typography variant="h5" key={key}>{children}</Typography>
      }
      case 'H5': {
        return <Typography variant="h6" key={key}>{children}</Typography>
      }
      case 'A': {
        if (node.attributes.href.value.includes('http')) {
          return <a key={key} href={node.attributes.href.value}>{children}</a>
        }
        return <Link to={node.attributes.href.value} key={key}>{children}</Link>
      }
      case 'OL': {
  ...
  return (
    <>
      ...
      <Container maxWidth="sm">
        <Img fluid={wpPost.featuredImage.node.localFile.childImageSharp.fluid} />
